...
## 68 LIVE, 77 BUILT — 13 August 2026

The era group (9) and the deep-cut group (6) went live as they were finished,
bringing the site to **68 published Quiz nodes**. The repo now holds **77
quizzes**; the nine tier and family editions are deployed to GitHub Pages and
are waiting on their Drupal nodes.

### The nine still to build

| Slug (`/quiz/…`) | Title | Difficulty | Questions |
|---|---|---|---|
| `badlands-expert` | The Dakota Papers | Challenging | 13 |
| `conservation-expert` | The Machinery of Conservation | Challenging | 14 |
| `presidency-expert` | The Instruments of Power | Challenging | 14 |
| `rough-riders-expert` | The First Volunteer Cavalry | Challenging | 13 |
| `myths-expert` | Did He Really Say That? | Challenging | 13 |
| `whole-life` | The Whole Life | Challenging | 15 |
| `early-life-family` | The Making of a Boy: Family Edition | Introductory | 9 |
| `conservation-family` | What He Saved: Family Edition | Introductory | 9 |
| `rough-riders-family` | The Rough Riders: Family Edition | Introductory | 9 |

Values for all 77 are in `build/drupal-nodes.csv`, regenerated by
`tools/drupal.py`.

### Two titles that had to change before building

`early-life-family` and `rough-riders-family` were originally titled exactly
like their parent quizzes — *The Making of a Boy* and *The Rough Riders*. Two
nodes with identical titles would have meant two indistinguishable rows in the
admin content list, which is precisely how five nodes were silently lost in the
calendar batch. All three family editions now carry a `: Family Edition`
suffix. **Before creating any node, check `build/drupal-nodes.csv` for a title
that already exists.**

### The gallery card is part of the quiz

Two things live in `index.json` rather than in the quiz file, and both are
public-facing text that can be wrong on its own:

- **The blurb doubles as the node's meta description.** When a quiz is
  corrected, its blurb does not follow. `badlands-expert`'s card promised a
  ledger "that settles what he really lost" for a day after the quiz itself
  had stopped claiming it — the card was overselling a certainty the quiz had
  withdrawn.
- **The title and blurb can give away answers.** `other-roosevelts` opened its
  card with its own first answer. The quiz titled *Sixteen Battleships* handed
  over a `presidency` question whose answer was "Sixteen battleships sent
  around the world"; that question was replaced rather than the title changed.

`tools/leaks.py` now reads titles, blurbs and intros for exactly this reason.
Run it before building nodes, not just before committing quizzes.
